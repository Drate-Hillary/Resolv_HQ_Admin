...
**Written:** 24 Sept 2026, updated 25 Sept 2026 (Week 4 of 8) | **Deadline:** 23 Oct 2026 | **Presentations:** 27/29/30 Oct 2026
...
This is not a new build — it's **already what `resolv-hq-backend` and `resolv-hq-customer` implement** (`agent-tools.ts`'s `search_knowledge_base` / `account_status_lookup` / `outage_status_checker` / `draft_escalation_ticket`, the `requests`/`knowledge_documents`/`agent_approvals` schema, the boundary matrix around refunds/billing/cancellations/account edits).

**Status: ✅ Resolved 25 Sept 2026.** Every place that described a procurement assistant instead (the frontend's scripted demo, its README, and all three AI documentation artifacts) has been rewritten to the support-request-triage domain — see [`../domain-conflict-resolution.md`](../domain-conflict-resolution.md) for the full account.
...
2. **Architecture diagram** — [`../architecture-diagram.md`](../architecture-diagram.md) now exists (drawn Week 4, covering through the tool-calling layer); keep extending this same file each week (Week 5's ReAct-loop detail is next) rather than starting fresh.
